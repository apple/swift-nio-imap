...
let commands: [(String, Command)] = [
    ...
    ("parse_esearch_simple_recursive_date", .extendedsearch(.init(key: .and([.not(.answered), .not(.before(NIOIMAP.IMAPDate(year: 2000, month: 12, day: 12)!))])))),
    ...
    ("parse_genurlauth_one", .generateAuthorizationURL([.init(urlRump: ByteBuffer(string: "test"), mechanism: .internal)])),
    ("parse_genurlauth_many", .generateAuthorizationURL([.init(urlRump: ByteBuffer(string: "test1"), mechanism: .internal), .init(urlRump: ByteBuffer(string: "test2"), mechanism: .internal), .init(urlRump: ByteBuffer(string: "test3"), mechanism: .internal)])),
