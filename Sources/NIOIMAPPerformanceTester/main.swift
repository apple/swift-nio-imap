...
let commands: [(String, Command)] = [
    ...
    ("parse_fetch_all_lots", .fetch(.set(.all), [.envelope, .flags, .internalDate, .gmailThreadID, .modificationSequence], [])),
    ...
    ("parse_auth_plain_initial data", .authenticate(mechanism: .init("PLAIN"), initialResponse: .init(ByteBuffer(string: "dGhpcyBpcyB0ZXN0IGJhc2U2NA==")))),
    ...
    ("parse_rename_params_one", .rename(from: .inbox, to: .init(ByteBuffer(string: "not an inbox")), parameters: ["name": nil])),
    ("parse_rename_params_many", .rename(from: .inbox, to: .init(ByteBuffer(string: "not an inbox")), parameters: ["name1": nil, "name2": .sequence(.set([1 ... 2, 3 ... 4]))])),
    ...
    ("parse_store_simple_last_command", .store(.lastCommand, [.unchangedSince(.init(modificationSequence: 124))], .flags(.add(silent: false, list: [.answered])))),
    ("parse_store_simple_all", .store(.set(.all), [.unchangedSince(.init(modificationSequence: 124))], .flags(.add(silent: false, list: [.answered])))),
    ("parse_store_complex_all", .store(.set(.all), [.unchangedSince(.init(modificationSequence: 124))], .flags(.add(silent: false, list: [.answered, .deleted, .flagged, .seen, .draft])))),
    ("parse_store_complex", .store(.set([1 ... 10, 11 ... 20, 21 ... 30, 31 ... 40]), [.unchangedSince(.init(modificationSequence: 124))], .flags(.add(silent: false, list: [.answered, .deleted, .flagged, .seen, .draft])))),
