...
/// within square brackets in the response text (for example, `[TRYCREATE]` or `[UIDVALIDITY 1234]`).
...
public indirect enum ResponseTextCode: Hashable, Sendable {
    ...
    ///
    /// Indicates that the accompanying text is a special alert that requires user
    ...
    ///
    /// Indicates that the operation failed because it would create a duplicate. For example,
    ...
    ///
    /// Indicates that a `SEARCH` command referenced a character set that the server does not
    ...
    ///
    /// May appear in the initial `OK` or `PREAUTH` response to transmit the server's capability list.
    /// Allows the client to learn the server's capabilities without sending a separate `CAPABILITY` command.
    /// See [RFC 3501](https://datatracker.ietf.org/doc/html/rfc3501) and
    /// [RFC 9051](https://datatracker.ietf.org/doc/html/rfc9051) for details.
    ...
    ///
    /// Indicates that the server encountered an error while parsing the [RFC-2822] headers
    ...
    ///
    /// Lists the flags that the client is allowed to store permanently in the selected mailbox.
    ...
    ///
    /// Indicates that the mailbox is selected in read-only mode, or that its access has
    ...
    ///
    /// Indicates that the mailbox is selected in read-write mode, or that its access has
    ...
    ///
    /// Suggests that the operation can succeed if the mailbox is first created using the
    ...
    ///
    /// Contains the UID that will be assigned to the next message appended to the selected mailbox.
    /// See [RFC 3501](https://datatracker.ietf.org/doc/html/rfc3501) and
    /// [RFC 9051](https://datatracker.ietf.org/doc/html/rfc9051) for details.
    case uidNext(UID)
    ...
    ///
    /// Contains the UIDVALIDITY value for the selected mailbox. The client must remember this
    ...
    ///
    /// Identifies the message number (in sequence order) of the first message in the mailbox
    ...
    ///
    /// Indicates that the command attempted to perform an operation in a namespace that the
    ...
    ///
    /// Returned in response to an `APPEND` command. It contains the `UIDVALIDITY` of the
    /// destination mailbox and the `UID` assigned to the newly appended message in that mailbox.
    /// Part of the UIDPLUS extension.
    ...
    ///
    /// Returned in response to a `COPY` command. It contains the `UIDVALIDITY` of the
    ...
    ///
    /// Indicates that the mail store does not support persistent UIDs; the `UIDVALIDITY`
    ...
    ///
    /// Indicates that the `CREATE` command failed because the server cannot create a mailbox
    /// with the specified special-use attribute. Part of the Special-Use Mailbox extension.
    ...
    ///
    /// A catch-all case that supports response codes that may be sent by future IMAP extensions or
    ...
    ///
    /// Indicates that a `SEARCH` (`SAVE`) command failed because the server has reached an
    /// internal limit on the number of saved search results.
    /// Part of the extended SEARCH extension.
    ...
    ///
    /// Appears when the server closes one mailbox and selects another. All responses
    ...
    ///
    /// Appears in the `OK` response to `SELECT` or `EXAMINE` commands when the server does not
    ...
    ///
    /// Returned in response to a `STORE` command and lists the messages that were actually
    /// modified. Allows the client to detect when the server rejected modifications for specific messages.
    /// See [RFC 4551](https://datatracker.ietf.org/doc/html/rfc4551) (CONDSTORE Extension) for details.
    case modified(LastCommandSet<UnknownMessageIdentifier>)

    /// Indicates the highest modification sequence value in the mailbox.
    ///
    /// Appears in `OK` responses to `SELECT` or `EXAMINE` commands when the server supports
    ...
    ///
    /// Indicates that a `SETMETADATA` command failed because an entry value is too large.
    ...
    ///
    /// Indicates that a `SETMETADATA` command failed because the server has reached the
    ...
    ///
    /// Indicates that a SETMETADATA command failed because the server does not allow
    ...
    ///
    /// Appears in `OK` responses to `RESETKEY`, `SELECT`, or `EXAMINE` commands. For `RESETKEY`,
    ...
    ///
    /// Returned in a `BYE` response when the server is not accepting connections and wishes
    ...
    ///
    /// Indicates that the server is temporarily unable to complete the command due to a
    ...
    ///
    /// Indicates that an authentication command failed, but the server is unwilling to
    ...
    ///
    /// Indicates that authentication succeeded with the authentication identity, but the
    /// server cannot or will not allow that identity to act as the requested authorization identity.
    /// Applies only when the authentication and authorization identities are different.
    /// See [RFC 5530](https://datatracker.ietf.org/doc/html/rfc5530) and
    /// [RFC 9051](https://datatracker.ietf.org/doc/html/rfc9051) for details.
    ...
    ///
    /// Indicates that either authentication succeeded but the server no longer has the
    ...
    ///
    /// Indicates that the operation cannot be completed without privacy protection. If
    ...
    ///
    /// Indicates that the user should contact the system administrator or support desk
    ...
    ///
    /// Indicates that the access control system (for example, ACL) does not permit the user to
    ...
    ///
    /// Indicates that the operation was not carried out because it involves removing a
    ...
    ///
    /// Indicates that another client has expunged messages from the selected mailbox.
    ...
    ///
    /// Indicates that the server has discovered that relevant data (such as mailbox data)
    ...
    ///
    /// Indicates that the server encountered a bug in itself or violated one of its own
    ...
    ///
    /// Indicates that the server has detected a client bug. This may accompany OK, NO, or BAD
    ...
    ///
    /// Indicates that the requested operation violates some invariant of the server and
    ...
    ///
    /// Indicates that the operation ran up against an implementation limit of some kind,
    ...
    ///
    /// Indicates that the user would exceed their quota if the operation completed. The
    ...
    ///
    /// Indicates that the operation failed because it tried to delete something that does not
    ...
    ///
    /// Indicates that the COMPRESS command has been issued and compression is now active
    ...
    ///
    /// Contains the MAILBOXID value for the selected mailbox. The client should remember
    /// this value to track mailboxes across sessions.
    /// Part of the OBJECTID extension.
