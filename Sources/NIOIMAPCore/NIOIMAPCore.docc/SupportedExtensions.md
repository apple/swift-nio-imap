...
Messages can also be searched by these identifiers: ``SearchKey/gmailMessageID(_:)`` and ``SearchKey/gmailThreadID(_:)`` correspond to the `X-GM-MSGID` and `X-GM-THRID` search keys, and ``SearchKey/gmailRaw(_:)`` sends an `X-GM-RAW` search using the same query syntax as the Gmail web interface.
