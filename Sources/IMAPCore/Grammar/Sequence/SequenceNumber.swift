...
extension IMAPCore.SequenceNumber: ExpressibleByIntegerLiteral {
...
// MARK: - Comparable
extension IMAPCore.SequenceNumber: Comparable {
    
    // last is treated as the largest possible element
    // i..e if the greatest mail ID is 5, then last is 5
    public static func < (lhs: IMAPCore.SequenceNumber, rhs: IMAPCore.SequenceNumber) -> Bool {
        switch (lhs, rhs) {
        case (.last, .last):
            return false
        case (.last, .number(_)):
