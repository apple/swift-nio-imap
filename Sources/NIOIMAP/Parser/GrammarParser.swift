...
extension NIOIMAP.GrammarParser {
    ...
    static func parseAddress(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Address {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Address in
            ...
            let name = try Self.parseAddressName(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString(" ", buffer: &buffer, tracker: tracker)
            let adl = try Self.parseAddressAdl(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString(" ", buffer: &buffer, tracker: tracker)
            let mailbox = try Self.parseAddressMailbox(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString(" ", buffer: &buffer, tracker: tracker)
            let host = try self.parseAddressHost(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString(")", buffer: &buffer, tracker: tracker)
    ...
    static func parseAppendData(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AppendData {
        ...
        func parseAppendData_literal(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AppendData {
            return .literal(try self.parseLiteralSize(buffer: &buffer, tracker: tracker))
        }
        
        func parseAppendData_literal8(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AppendData {
            return .literal8(try self.parseLiteral8Size(buffer: &buffer, tracker: tracker))
        }
        
        func parseAppendData_dataExtension(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AppendData {
            return .dataExtension(try self.parseAppendDataExtension(buffer: &buffer, tracker: tracker))
        }
        
        return try ParserLibrary.parseOneOf([
            parseAppendData_literal,
            parseAppendData_literal8,
            parseAppendData_dataExtension
    ...
    static func parseAString(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AString {
        func parseOneOrMoreASTRINGCHAR(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.AString {
...
//            } ?? []
    ...
    static func parseBase64(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Base64 {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Base64 in
    ...
    static func parseBodyExtSinglePart(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.ExtensionSinglepart {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Body.ExtensionSinglepart in
            let md5 = try self.parseBodyFieldMd5(buffer: &buffer, tracker: tracker)
    ...
    static func parseBodyFields(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.Fields {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Body.Fields in
            ...
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let fieldID = try self.parseBodyFieldId(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let fieldDescription = try self.parseBodyFieldDescription(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
    ...
    static func parseBodyFieldDescription(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.NString {
        return try self.parseNString(buffer: &buffer, tracker: tracker)
    }
    ...
    static func parseBodyFieldLanguage(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldLanguage {
        ...
        func parseBodyFieldLanguage_multiple(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldLanguage {
            ...
            try ParserLibrary.parseZeroOrMore(buffer: &buffer, into: &array, tracker: tracker) { (buffer, tracker) -> NIOIMAP.IMAPString in
                try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
    ...
    static func parseBodyFieldLocation(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.NString {
        return try self.parseNString(buffer: &buffer, tracker: tracker)
    }
    ...
    static func parseBodyFieldParam(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldParameter {

        func parseBodyFieldParam_nil(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldParameter {
            try parseNil(buffer: &buffer, tracker: tracker)
            return nil
        }

        func parseBodyFieldParam_pairs(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldParameter {
            try ParserLibrary.parseFixedString("(", buffer: &buffer, tracker: tracker)
    ...
    static func parseEnvelope(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Envelope {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Envelope in
            try ParserLibrary.parseFixedString("(", buffer: &buffer, tracker: tracker)
            let date = try self.parseEnvelopeDate(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let subject = try self.parseEnvelopeSubject(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            ...
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let inReplyTo = try self.parseEnvelopeInReplyTo(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let messageID = try self.parseEnvelopeMessageId(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString(")", buffer: &buffer, tracker: tracker)
    ...
    static func parseHeaderFieldName(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.HeaderFieldName {
        return try self.parseAString(buffer: &buffer, tracker: tracker)
    }

    // header-list     = "(" header-fld-name *(SP header-fld-name) ")"
    static func parseHeaderList(buffer: inout ByteBuffer, tracker: StackTracker) throws -> [NIOIMAP.HeaderFieldName] {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> [NIOIMAP.HeaderFieldName] in
            try ParserLibrary.parseFixedString("(", buffer: &buffer, tracker: tracker)
            var output = [try self.parseHeaderFieldName(buffer: &buffer, tracker: tracker)]
            try ParserLibrary.parseZeroOrMore(buffer: &buffer, into: &output, tracker: tracker) { (buffer, tracker) -> NIOIMAP.HeaderFieldName in
                try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
    ...
    static func parseLiteral(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Literal {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Literal in
            try ParserLibrary.parseFixedString("{", buffer: &buffer, tracker: tracker)
    ...
    static func parseLiteral8(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Literal {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Literal in
            try ParserLibrary.parseFixedString("~{", buffer: &buffer, tracker: tracker)
            let length = try Self.parseNumber(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseFixedString("}", buffer: &buffer, tracker: tracker)
    ...
    static func parseMessageAttributeStart(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.MessageAttributeType {
        try ParserLibrary.parseFixedString("(", buffer: &buffer, tracker: tracker)
        return try self.parseMessageAttribute_dynamicOrStatic(buffer: &buffer, tracker: tracker)
    }
    ...
    static func parseMessageAttributeMiddle(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.MessageAttributeType? {
        let att = try ParserLibrary.parseOptional(buffer: &buffer, tracker: tracker) { (buffer, tracker) -> NIOIMAP.MessageAttributeType in
            try ParserLibrary.parseFixedString(" ", buffer: &buffer, tracker: tracker)
            return try self.parseMessageAttribute_dynamicOrStatic(buffer: &buffer, tracker: tracker)
        }
        try ParserLibrary.parseOptional(buffer: &buffer, tracker: tracker) { (buffer, tracker) in
    ...
    static func parseMessageAttributeStatic(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.MessageAttributesStatic {
        ...
        func parseMessageAttributeStatic_binaryString(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.MessageAttributesStatic {
            try ParserLibrary.parseFixedString("BINARY", buffer: &buffer, tracker: tracker)
            let section = try self.parseSectionBinary(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let nstring = try self.parseNString(buffer: &buffer, tracker: tracker)
            return .binaryString(section: section, string: nstring)
        }

        func parseMessageAttributeStatic_binaryLiteral(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.MessageAttributesStatic {
            try ParserLibrary.parseFixedString("BINARY", buffer: &buffer, tracker: tracker)
            let section = try self.parseSectionBinary(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            let size = try self.parseLiteral8Size(buffer: &buffer, tracker: tracker)
        ...
        return try ParserLibrary.parseOneOf([
            ...
            parseMessageAttributeStatic_binaryString,
            parseMessageAttributeStatic_binaryLiteral
        ], buffer: &buffer, tracker: tracker)
    }
    ...
    // password        = astring
    static func parsePassword(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Password {
        return try Self.parseAString(buffer: &buffer, tracker: tracker)
    }
    ...
    static func parseQuoted(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Quoted {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.Quoted in
            try ParserLibrary.parseFixedString("\"", buffer: &buffer, tracker: tracker)
    ...
    static func parseSectionBinary(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.SectionBinary {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> NIOIMAP.SectionBinary in
            try ParserLibrary.parseFixedString("[", buffer: &buffer, tracker: tracker)
    ...
    static func parseString(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.IMAPString {
        return try ParserLibrary.parseOneOf([
            Self.parseQuoted,
            Self.parseLiteral
        ], buffer: &buffer, tracker: tracker)
    ...
    // text            = 1*TEXT-CHAR
    static func parseText(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Text {
        return try ParserLibrary.parseOneOrMoreCharactersByteBuffer(buffer: &buffer, tracker: tracker) { char -> Bool in
...
extension NIOIMAP.GrammarParser {
    ...
    static func parseBodyLocationExtension(buffer: inout ByteBuffer, tracker: StackTracker) throws -> NIOIMAP.Body.FieldLocationExtension {
        let fieldLocation = try self.parseBodyFieldLocation(buffer: &buffer, tracker: tracker)
        let extensions = try ParserLibrary.parseZeroOrMore(buffer: &buffer, tracker: tracker) { (buffer, tracker) -> NIOIMAP.BodyExtension in
            try ParserLibrary.parseSpace(buffer: &buffer, tracker: tracker)
            return try self.parseBodyExtension(buffer: &buffer, tracker: tracker)
        }
    ...
    static func parseLiteral8Size(buffer: inout ByteBuffer, tracker: StackTracker) throws -> Int {
        return try ParserLibrary.parseComposite(buffer: &buffer, tracker: tracker) { buffer, tracker -> Int in
            try ParserLibrary.parseFixedString("~{", buffer: &buffer, tracker: tracker)
            let length = try Self.parseNumber(buffer: &buffer, tracker: tracker)
            try ParserLibrary.parseOptional(buffer: &buffer, tracker: tracker) { (buffer, tracker) in
