...
Alongside ``IMAPConnection/send(_:_:)``, the module provides dedicated APIs for the commands that don’t fit the simple request/response shape: ``IMAPConnection/sendIdle(_:)`` for `IDLE`, ``IMAPConnection/sendAuthenticate(mechanism:initialResponse:_:)`` for `AUTHENTICATE` challenge/response, and ``IMAPConnection/append(to:writeClosure:readClosure:)`` for streaming an `APPEND`.
