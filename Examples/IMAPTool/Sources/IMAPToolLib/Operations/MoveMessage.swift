...
extension ConnectionProtocol {
    ...
    private func moveMessagesUsingMoveCommand(
    ...
    private func moveMessagesUsingCopyCommand(
        uids: MessageIdentifierSetNonEmpty<UID>,
        to destinationMailbox: MailboxName
    ) async throws {
        // UID COPY
        let copyText = try await send(
        ...
        // EXPUNGE
        let expungeText = try await send(.expunge) { tag, responses in
