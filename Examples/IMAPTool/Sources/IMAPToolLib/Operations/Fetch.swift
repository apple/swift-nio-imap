...
extension ConnectionProtocol {
    ...
    func batchedFetchSimpleAttributes<Result: Sendable>(
        mailboxMessageCount: Int,
        capabilities: [Capability],
        query: FetchQuery,
        ...
        return try await batched(
            ...
            capabilities: capabilities,
            fetch: { batch in
                let r: [UID: [MessageAttribute]] = try await self.fetchSimpleAttributes(
                    capabilities: capabilities,
...
extension ConnectionProtocol {
    ...
    func fetchSimpleAttributes<Result: Sendable>(
        capabilities: [Capability],
        uids: UIDSet,
...
extension ConnectionProtocol {
    ...
    func fetch<Result: Sendable>(
        ...
        return try await send(command) { tag, responses -> Result in
