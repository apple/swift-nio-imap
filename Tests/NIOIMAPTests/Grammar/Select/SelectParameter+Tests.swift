...
extension SelectParameter_Tests {
    ...
    func testEncode() {
        ...
        for (input, expectedString, line) in inputs {
            ...
            let size = self.testBuffer.writeSelectParameter(input)
    ...
    func testEncode_multiple() {
        let inputs: [(NIOIMAP.SelectParameters, String, UInt)] = [
            ([.name("some", value: nil)], " (some)", #line),
        ...
        for (input, expectedString, line) in inputs {
            self.testBuffer.clear()
            let size = self.testBuffer.writeSelectParameters(input)
            XCTAssertEqual(size, expectedString.utf8.count, line: line)
            XCTAssertEqual(self.testBufferString, expectedString, line: line)
        }
    }
