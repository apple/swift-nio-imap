...
extension IMessagePart_Tests {
    func testEncode() {
        let inputs: [(IMessagePart, String, UInt)] = [
            ...
            (
                .init(mailboxReference: .init(encodeMailbox: .init(mailbox: "test")), iUID: try! .init(uid: 123), iSection: nil, iPartial: .init(range: .init(offset: 123, length: 4))),
                "test/;UID=123/;PARTIAL=123.4",
                #line
            ),
            (
                .init(mailboxReference: .init(encodeMailbox: .init(mailbox: "test")), iUID: try! .init(uid: 123), iSection: .init(encodedSection: .init(section: "section")), iPartial: .init(range: .init(offset: 123, length: 4))),
