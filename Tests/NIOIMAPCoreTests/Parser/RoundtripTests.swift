...
private enum RoundtripTests {
    ...
    @Test(arguments: [
        ...
        RoundtripFixture(name: "UNSUBSCRIBE command with mailbox name", command: .unsubscribe(MailboxName("mailbox"))),
        ...
        RoundtripFixture(name: "LSUB command with INBOX reference", command: .lsub(reference: .inbox, pattern: "abcd")),
        RoundtripFixture(
        ...
        RoundtripFixture(name: "STATUS command with INBOX and single item", command: .status(.inbox, [.messageCount])),
        RoundtripFixture(
